...
from utils.utils_xml import get_detection_areas, xml_from_alarm
...
def video_analyzer(analysis_queue, 
                   video_mapping_path, 
                   class_list, 
                   target_fps, 
                   target_fps_PeopleCounting, 
                   yolo_model, 
                   test_mode, 
                   skip_time):
    """
    ...
    while True:
        ...
        if any('PeopleCounting' in area for area in areas_keys_list):
            if fps > target_fps_PeopleCounting:
                frame_skip = int(fps / target_fps_PeopleCounting)
            else:
                frame_skip = 1
