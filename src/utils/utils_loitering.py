...
class Loitering:
    ...
    def __init__(self, 
        ...
        self.Loitering_area = areas['Loitering']
        
        self.alarm_prediction = []
        
        self.Loitering_track_boxes = defaultdict(lambda: [])
        self.Loitering_track_mapping_ids = defaultdict(lambda: -1)
        self.Loitering_track_frame = defaultdict(lambda: [])
        self.Loitering_track_in_check = defaultdict(lambda: [])
        self.loitering_check = [0, 0, 0, 0]
        self.Loitering_non_count = 0
        self.Loitering_duration = None
        self.Loitering_duration_frame = 0
    ...
    def process_results(self, 
        ...
        Loitering_remind_boxes = []
        box_track_LI(results, 
                    self.Loitering_track_boxes, 
                    self.Loitering_track_mapping_ids, 
                    self.Loitering_track_frame, 
                    frame_count, 
                    self.Loitering_track_in_check, 
                    Loitering_remind_boxes, 
                    self.Loitering_area, 
        ...
        Loitering_params = [self.Loitering_non_count, 
                            self.Loitering_duration, 
                            self.start_time, 
                            self.Loitering_duration_frame]
        
        self.loitering_check, self.start_time, Loitering_params = alarm_LI(
            self.alarm_prediction, 
            'Loitering', 
            self.Loitering_track_frame, 
            self.Loitering_track_boxes, 
        ...
        self.Loitering_non_count, self.Loitering_duration, self.start_time, self.Loitering_duration_frame = Loitering_params
    ...
    def postprocess(self, 
        ...
        if self.loitering_check[1] != 0 or self.loitering_check[3] != 0:
                
            if self.Loitering_duration != None:
                hours, minutes, seconds = self.Loitering_duration.split(":")
                hours, minutes, seconds = int(hours), int(minutes), int(seconds)
                if hours > 0 or minutes > 0 or seconds > 10:
                    ...
                    duration_obj = datetime.strptime(self.Loitering_duration, time_format)
                    new_duration_obj = duration_obj - timedelta(seconds=10)
                    new_duration = new_duration_obj.strftime(time_format)
